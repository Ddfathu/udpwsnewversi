...
// NETWORK TYPES
const MUX_NETWORK_TCP = 0x01; // Tambahan support TCP
...
async function readMuxFrame(reader) {
    ...
    else if (frame.status === MUX_STATUS_KEEP && meta.length > cursor && (meta[cursor] === MUX_NETWORK_UDP || meta[cursor] === MUX_NETWORK_TCP)) {
        frame.network = meta[cursor++];
...
class MuxSession {
    constructor(mux, id, network, target) {
        ...
        this.target = target;
        this.udp = null;
        this.tcpSocket = null; // Penanganan koneksi outbound TCP
        this.global = false;
        this.gid = null;
        this.closed = false;
    }
    ...
    sendTCP(payload) {
        if (this.closed || !this.tcpSocket || this.tcpSocket.destroyed) {
            return;
        }
...
class MuxConnection {
    ...
    async handleNew(frame) {
        if (!frame.target?.host || !frame.target?.port) {
            await this.sendEnd(frame.id, true).catch(() => { });
            return;
        }
        
        // --- PROSESSING TCP (Web Proxy Standar) ---
        if (frame.network === MUX_NETWORK_TCP) {
            this.removeSession(frame.id);
            const session = new MuxSession(this, frame.id, frame.network, frame.target);
            
            const tcpSocket = net.createConnection({ host: frame.target.host, port: frame.target.port });
            session.tcpSocket = tcpSocket;
            this.sessions.set(session.id, session);

            tcpSocket.on('data', (chunk) => {
                this.sendTCPData(session.id, chunk).catch(() => session.close(true));
            });

            tcpSocket.on('error', () => session.close(true));
            tcpSocket.on('close', () => session.close(true));

            if (frame.data.length) {
                tcpSocket.once('connect', () => session.sendTCP(frame.data));
            }
            return;
        }
    ...
    sendTCPData(id, data) {
        const meta = Buffer.allocUnsafe(4);
...
<head>
    ...
    <title>Railway UDP/TCP Relay Status</title>
    <style>
...
<body>
    <div class="wrapper">
        <div class="header">
            <div>
                <div class="title">XUDP & TCP Dual Relay</div>
...
async function main() {
    ...
    console.log(`TCP & UDP WebSocket relay listening on ${scheme}://${addr.address}:${addr.port}${path} auth=none xudp-grace=${cfg.xudpGrace}ms`);
